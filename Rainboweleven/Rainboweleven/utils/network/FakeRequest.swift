...
struct FakeRequest {
    let response: Any?
    let responseType:RBNetworking.ResponseType
    let statusCode: Int

    static func find(ofType type:RBNetworking.RequestType, forPath path: String, in collection: [RBNetworking.RequestType: [String: FakeRequest]]) -> FakeRequest? {
