...
public extension Int {
    ...
    var statusCodeType:RBNetworking.StatusCodeType {
        switch self {
...
open class RBNetworking {
    static let domain = "com.r11.networking"
    ...
    public func composedURL(with path: String) throws -> URL {
        ...
        guard let url = URL(string: baseURL + encodedPath) else {
            throw NSError(domain:RBNetworking.domain, code: 0, userInfo: [NSLocalizedDescriptionKey: "Couldn't create a url using baseURL: \(baseURL) and encodedPath: \(encodedPath)"])
        }
        return url
    }
    ...
    public func destinationURL(for path: String, cacheName: String? = nil) throws -> URL {
        ...
        let folderPath = RBNetworking.domain
        ...
        if let url = URL(string: finalPath) {
            ...
            if let cachesURL = FileManager.default.urls(for: directory, in: .userDomainMask).first {
                ...
                throw NSError(domain:RBNetworking.domain, code: 9999, userInfo: [NSLocalizedDescriptionKey: "Couldn't normalize url"])
            }
        } else {
            throw NSError(domain:RBNetworking.domain, code: 9999, userInfo: [NSLocalizedDescriptionKey: "Couldn't create a url using replacedPath: \(finalPath)"])
        }
    }
    ...
    public static func deleteCachedFiles() {
        ...
        #endif
        if let cachesURL = FileManager.default.urls(for: directory, in: .userDomainMask).first {
            let folderURL = cachesURL.appendingPathComponent(URL(string:RBNetworking.domain)!.absoluteString)
    ...
    public func reset() {
       ...
       RBNetworking.deleteCachedFiles()
    }
}
