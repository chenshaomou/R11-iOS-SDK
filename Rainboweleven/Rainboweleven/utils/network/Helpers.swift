    ...
    public func urlEncodedString() throws -> String {
        ...
        let pairs = try reduce([]) { current, keyValuePair -> [String] in
            if let encodedValue = "\(keyValuePair.value)".addingPercentEncoding(withAllowedCharacters: .urlQueryParametersAllowed) {
                ...
                throw NSError(domain:RBNetworking.domain, code: 0, userInfo: [NSLocalizedDescriptionKey: "Couldn't encode \(keyValuePair.value)"])
            }
        }
...
extension URLRequest {
    init(url: URL, requestType:RBNetworking.RequestType, path _: String, parameterType:RBNetworking.ParameterType?, responseType:RBNetworking.ResponseType, boundary: String, authorizationHeaderValue: String?, token: String?, authorizationHeaderKey: String, headerFields: [String: String]?) {
...
extension NSError {
    convenience init(fakeRequest: FakeRequest) {
        self.init(domain:RBNetworking.domain, code: fakeRequest.statusCode, userInfo: [NSLocalizedDescriptionKey: HTTPURLResponse.localizedString(forStatusCode: fakeRequest.statusCode)])
    }
}
