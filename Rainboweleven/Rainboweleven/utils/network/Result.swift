...
public enum ImageResult: Result {
    ...
    public init(body: Any?, response: HTTPURLResponse, error: NSError?) {
        ...
        if let error = error {
            ...
            let error = NSError(domain:RBNetworking.domain, code: URLError.cannotParseResponse.rawValue, userInfo: [NSLocalizedDescriptionKey: "Malformed image"])
            self = .failure(FailureResponse(response: response, error: error))
        }
    }
}
...
public enum DataResult: Result {
    ...
    public init(body: Any?, response: HTTPURLResponse, error: NSError?) {
        ...
        if let error = error {
            ...
            let error = NSError(domain:RBNetworking.domain, code: URLError.cannotParseResponse.rawValue, userInfo: [NSLocalizedDescriptionKey: "Malformed data"])
            self = .failure(FailureResponse(response: response, error: error))
        }
    }
}
...
extension JSONResult {
    ...
    public func toJSONString() ->String {
        switch self {
        case .failure(let rsp):
            return String(data: rsp.data, encoding: .utf8) ?? ""
